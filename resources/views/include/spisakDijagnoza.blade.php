...
<div class="container">
	<div class="row">
		<h3 class="inline"><p class="naslov">Pregled Dijagnoza Pacijenta</p></h3>
		<table class="table">
			<tbody>
				<tr>
					<td>
...
<div class="col-md-10 col-md-offset-1">
	<table class="table table-striped">
		...
		<tbody>
			@foreach($data as $da)
			<tr>
				<td class="wrap">{{$da->dijagnoza}}</td>
				<td class="wrap">{{$da->name}}</td>
				<td class="wrap">{{date('F d, Y', strtotime($da->created_at))}}</td>
				<td class="wrap">
					<a href="{{url('jednaDijagnoza/'.$da->id)}}"><button type="button" class="btn btn-primary">Prikazi Dijagnozu</button></a>
				</td>
			</tr>
