    ...
    <div class="row">
        <div class="col-md-8 col-md-offset-2">
            <div class="panel panel-default">
                <div class="panel-heading">Promena Podataka Doktora</div>
                @if(session('success'))
                <p class="zeleno" id="sakri">{{session('success')}}</p>
                @endif
                <div class="panel-body">
                    @foreach($data as $da)
                    <form class="form-horizontal" role="form" method="POST" action="{{ url('upisiPromenuDoktora/'.$da->id) }}">
                        {{ csrf_field() }}
                        <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
                            <label for="name" class="col-md-4 control-label">Ime i Prezime</label>
                            <div class="col-md-6">
                                <input id="name" type="text" class="form-control" name="name" value="{{ $da->name }}">
                                @if ($errors->has('name'))
                        ...
                        <div class="form-group{{ $errors->has('email') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6">
                                <input id="email" type="email" class="form-control" name="email" value="{{ $da->email }}">
                                @if ($errors->has('email'))
                                <span class="help-block">
