...
<div class="panel panel-default">
  <div class="panel-heading textSredina"><b>Nastavak Lečenja od {{date('F d, Y', strtotime($da->created_at))}}</b></div>
...
<div class="col-md-10 col-md-offset-1">
  ...
  <p class="zeleno" id="sakri">{{session('success')}}</strong>
    @endif
    <form  class="form-horizontal" role="form" method="post" action="{{ url('azurirajLecenje/'.$da->id) }}">
      
      <div class="form-group">
        <label for="simptomi"> Simptomi</label>
        <textarea class="form-control" rows="4"  id="simptomi" name="simptomi" >{{$da->simptomi}}</textarea>
      </div>
      ...
      <div class="form-group">
        ...
        <textarea class="form-control" rows="4" id="comment" name="dijagnoza" >{{$da->dijagnoza}}</textarea>
      </div>
      ...
      <div class="form-group">
        ...
        <textarea class="form-control" rows="4" id="comment" name="lecenje" >{{$da->lecenje}}</textarea>
      </div>
      ...
      <div class="form-group">
        ...
        <textarea class="form-control" rows="4" id="lek" name="lek" >{{$da->lek}}</textarea>
      </div>
      @if ($errors->has('lek'))
      <span class="help-block">
      ...
      <div class="dugmeStampa"><a href="{{ url('printNastavak') }}" class="stampa"> Štampa</a></div>   </div>
    </div>
  </div>
  @endforeach
  @endsection
